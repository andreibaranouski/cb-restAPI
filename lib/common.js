...
//                              logger.info("not json format of response body", options.path, body);
      ...
      post_api: function (t, protocol, post_data, options, callback) {
          ...
          var req = protocol.request(options, function (response) {
              ...
              response.on('end', function () {
                  logger.info(response.statusCode + " from http://" + options.host + ":" + options.port + options.path, "data:", post_data);
  ...
  sendRequests: function(t, paths, options, status, stream) {
	  ...
	  sendPostRequests: function(t, paths, post_data, options, status, stream) {
		    async.times(paths.length, function (i, next) {
		            obj = extend({}, options);
		            obj.path=paths[i];
		            obj.path = paths[i];
		            common.post_api(t, protocol, post_data[i], obj, function (callback) {}, next);
		        }, function () {},
		        setTimeout(function () {
		            console.log("sleep 5 sec");
		            t.end();
